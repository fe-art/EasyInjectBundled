...
public class InstallFlow extends InstallerWindow.ScreenPanel {
    ...
    private class SecurityStepPanel extends JPanel {
        ...
        private void updateContinueState() {
            boolean canContinue = defenderPassed || defenderSkipped;
        ...
        private void runInstall() {
            ...
            new SwingWorker<String, String>() {
                ...
                protected String doInBackground() {
                    try {
                        // MCSR guard — manual install is not supported for MCSR Launcher instances.
                        if (Main.isMcsrLauncherInstance(window.getInstanceDir())) {
                            return "This instance is managed by MCSR Launcher. "
                                + "Enable Toolscreen from the launcher's instance tools instead.";
                        }
                        ...
                        // Modrinth App branch — route the prelaunch hook through the Modrinth DB.
                        Main.ModrinthInstance modrinth = window.getModrinthInstance();
                        if (modrinth != null) {
                            ...
                            // Step 4: Write prelaunch hook into the Modrinth profile.
                            publish("write");
                            Main.InstallResult r = Main.installForModrinthProfile(window.getStableJar(), modrinth);
                            if (!r.success) {
                                return r.error;
                            }
                        ...
                        if (isJson) {
                            // ATLauncher: ensure the launcher is closed before rewriting instance.json.
                            // ensureClosedInteractive() shows a modal dialog, so run it on the EDT.
                            final boolean[] atlCancelled = { false };
                            try {
                                javax.swing.SwingUtilities.invokeAndWait(new Runnable() {
                                    @Override
                                    public void run() {
                                        if (!com.easyinject.AtLauncherSupport.ensureClosedInteractive()) {
                                            atlCancelled[0] = true;
                                        }
                                    }
                                });
